...
<script type="text/javascript">
	...
	$(function() {
			...
			for (let i = 0; i < address.length; i++) {
				...
				geocoder.addressSearch(address[i], function(result, status) {
					...
					if (status === kakao.maps.services.Status.OK) {
						...
						var iwContent = '<div style="width:50px; height:200px">'+feature[i]+'</div>', // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
		...
		//셀렉트가 수행되지 않는 경우
		$.ajax({
					url: "resources/data/MOCK_DATA (2)5.json",
					success: function(data) {
						...
						function paging(totalData, dataPerPage, pageCount, currentPage) {
							...
							for (var i = 0; i < 8; i++) { //24 ~ 28
								...
								list += "<div class = 'pp'>"
								list += "<img src = 'resources/img/home.jpg' style = 'width: 200px; height: 120px'>"
								list += "<div style='margin-left: 20px'>" + data[i].id+ "</div>"
								list += "<div style='margin-left: 20px'>" + data[i].city+ "</div>"
								list += "</div>"
								list += "</div>"
								}
								else{
...
<body>
	...
	<section class="content">
		...
		<div id="productSummary">
			<div id = "list"></div>
			<div id="paging" style='margin-left: 250px'></div>
		</div>
